...
sp-LLaMA-Factory adds Sequence Parallelism (SP), more specifically zigzag-ring-attention, into LLaMA-Factory, supporting most post-training methods including SFT and DPO (plus its variants).
...
We believe SP is of great significance as multi-modal LLMs process longer sequences of tokens, and sp-LLaMA-Factory patches this critical missing piece into the most widely-used LLM Post-Training framework.
Our SP implementation in sp-LLaMA-Factory introduces minimal code change, is highly modular and has its correctness verified.
...
Before that, this codebase could be used directly in place of the original LLaMA-Factory codebase -- sp-LLaMA-Factory's behavior is *identical* to the original one without SP.
...
Upon initial release of sp-LLaMA-Factory (Dec. 30, 2024),
we have developed sp-LLaMA-Factory based on the latest release v0.9.1 of LLaMA-Factory. 
We will keep up with LLaMA-Factory's major later releases.

### Installation
To run sp-LLaMA-Factory with SP, 
...
#### Fresh-Start
We have updated `requirements.txt`, so you could simply create a new Python environment and install sp-LLaMA-Factory.

```shell
conda create -n sp-LLaMA-Factory python=3.11 -y  # LLaMA-Factory recommends Python 3.11
conda activate sp-LLaMA-Factory

git clone https://github.com/Qihoo360/sp-LLaMA-Factory.git 
cd sp-LLaMA-Factory
...
You could also refer to [sp-example.sh](sp-example.sh) for example SFT and DPO scripts.
...
| sp-LLaMA-Factory     | ✅     | ✅                                    | zigzag ring attention         | easier plug&play use, less customizable | ring-flash-attn, trl, etc                     | ✅ (inherits transformers)                                            |


To the best of our knowledge, sp-LLaMA-Factory is the first open-source project to systematically integrate Sequence Parallelism (SP) into LLaMA-Factory.
...
- [OpenRLHF](https://github.com/OpenRLHF/OpenRLHF) is arguably the only alternative choice we would recommend besides sp-LLaMA-Factory as its implementation is also correct with active community support. In terms of software engineering, OpenRLHF is on the opposite end of sp-LLaMA-Factory: OpenRLHF has much fewer dependencies with self-written Trainer classes instead of inheriting `tansformers`'s Trainer, while sp-LLaMA-Factory, as LLaMA-Factory, inherits much from `trl`, `transformers` and other libraries. Generally OpenRLHF is more suitable for more advanced custom use and modification, while sp-LLaMA-Factory for general-purpose argument-specifiable use (e.g. easily combined with unsloth or liger-kernel).

While our detailed implementation of data processing and loss computation is different from all above, 
we gratefully acknowledge these projects' pioneering work. In fact, we have been internally using our own bespoke version of EasyContext for SP post-training throughout the year, and have only recently decided to migrate our modification on EasyContext into LLaMA-Factory. The SP backend EasyContext and sp-LLaMA-Factory used, [ring-flash-attention](https://github.com/zhuzilin/ring-flash-attention), is also the same as OpenRLHF.
We also plan to add other SP modes such as DeepSpeed Ulysses as XTuner did.


## Benchmarking sp-LLaMA-Factory
...
We have been post-training SFT and DPO with SP without any notable issues on the resulting model's performance. We are confident sp-LLaMA-Factory *works and is correct*.
...
### Modularity
We believe our SP implementation in sp-LLaMA-Factory introduces minimal code change and is highly modular.
